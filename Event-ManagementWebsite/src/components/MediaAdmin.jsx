...
const MediaAdmin = () => {
  ...
  const Addmediabutton = async () => {
    ...
    try {
      ...
      let response = await fetch("https://react-pro-front.onrender.com/Addmedia", {
        method: "POST",
        body: formdata
      })
  ...
  let removemedia = async (id) => {
    let removemedia = await fetch(`https://react-pro-front.onrender.com/mediadelete/${id}`,{
      method:"DELETE"
    })
  ...
  useEffect(() => {
    let getmediadata = async () => {
      let getmediadata = await fetch("https://react-pro-front.onrender.com/fetchmedia")
