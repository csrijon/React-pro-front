...
const FirstAboutSection = () => {
  ...
  useEffect(() => {
    let fetchmainabout = async () => {
      try {
        let response = await fetch("https://backendof-event.onrender.com/api/fetchaboutmain");
    ...
    let fetchaboutstarts = async () => {
      try {
        let response = await fetch("https://backendof-event.onrender.com/api/fetchaboutstats");
        let data = await response.json();
